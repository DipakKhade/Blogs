...
const TracingBeam = ({ children, className }) => {
  ...
  const y1 = useSpring(useTransform(scrollYProgress, [0, 0.8], [50, svgHeight - velo * 500]), {
    stiffness: 500,
    damping: 90,
  });

  const y2 = useSpring(useTransform(scrollYProgress, [0, 1], [50, svgHeight - velo * 2000]), {
    stiffness: 500,
    damping: 90,
  });
  ...
  return (
    ...
    <motion.div className={cn("relative w-full max-w-4xl mx-auto h-[50vh]", className)}>
      <div className="absolute -left-20 top-3">
        ...
        <svg viewBox={`0 0 20 ${svgHeight}`} width="20" height={svgHeight} className="ml-4 hidden lg:block" aria-hidden="true">
          <motion.path
